...
# vpc_list=$(aws ec2 describe-vpcs --region $region 2>/dev/null | jq '.Vpcs[].VpcId' -r), $(echo " ") # to store the output of the command
# ${} is used when you are using variable name ex ${NAME}
# ${#vpc_array[@]}, echo ${NAME} # to print the value of the variable
